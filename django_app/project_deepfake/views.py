...
class validation_dataset(Dataset):
    ...
    def __getitem__(self,idx):
        ...
        frames = torch.stack(frames).to(device)  # Move frames to MPS or CPU as needed
...
def index(request):
    if request.method == 'GET':
        ...
        if video_upload_form.is_valid():
            ...
            return redirect('project_deepfake:predict')
        else:
            return render(request, index_template_name, {"form": video_upload_form})
...
def get_video_metadata(video_file):
    ...
    # Get basic file properties (size, creation time)
    ...
    # Default values in case video stream is not found
    resolution = frame_rate = codec = duration = "N/A"

    # Get video stream metadata using ffmpeg
    try:
        # Probe only the video stream
        video_probe = ffmpeg.probe(video_file, v='error', select_streams='v:0', show_entries='stream=codec_name,width,height,avg_frame_rate,duration')

        # Extract video stream metadata
        video_stream = video_probe['streams'][0]  # There should be only one video stream if it's selected correctly
        resolution = f"{video_stream['width']}x{video_stream['height']}"
        frame_rate = eval(video_stream['avg_frame_rate'])  # avg_frame_rate is a string like "30000/1001"
        codec = video_stream['codec_name']
        duration = video_stream.get('duration', 'N/A')  # Get video duration if available

    except ffmpeg.Error as e:
        print(f"Error: {e.stderr.decode()}")  # Print stderr for debugging
        return None

    # Populate metadata dictionary with the extracted values
    ...
    # Debugging output to ensure the metadata is correct
    print("Extracted video metadata:", metadata)
...
def predict_page(request):
    if request.method == "GET":
        ...
        if 'file_name' not in request.session:
            return redirect("project_deepfake:home")
        if 'file_name' in request.session:
        ...
        # Check for MPS availability and initialize the model accordingly
        if torch.backends.mps.is_available():
            device = torch.device("mps")
        else:
            device = torch.device("cpu")
            # Ensure the model is instantiated and moved to the right device
        model = Model(2).to(device)  # Initialize the model first
        ...
        try:
            ...
            for i in range(len(path_to_videos)):
                ...
                processing_time = round(time.time() - start_time, 2)  # Calculate the processing time
                video_metadata = get_video_metadata(video_file) # Get video metadata
...
def homepage(request):
    return render(request, 'homepage.html')  # Render the new page template
